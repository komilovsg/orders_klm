...
Готовы лендинг, обзор и раздел «Общее» целиком: реестр позиций, поставщики, плательщики и цены закупки.

**Цены закупки** собраны из `istochniki/ЦЕНООБРАЗОВАНИЕ/`: 17 материалов с поставщиками и историей цен, 11 итогов переговоров с разбивкой на металлы, прочее и сроки поставки, плюс котировки сырья — медь, алюминий и курс. Эти же котировки выведены на шину в шапке: цена металла и есть основание закупки.

**Плательщики** — 7 компаний. Таблицы в источниках нет, реквизиты лежат отдельными PDF и DOCX, поэтому справочник собран из имён файлов: компания, банк, дата актуальности. Полные реквизиты остаются внутри документов.

Этапы «Заказ», «План оплат» и «Тендер» показывают **макет**: примерный вид раздела с явной пометкой, что данные придуманы, и разбором, что нужно загрузить для настоящих. Там же — загрузка своего Excel: файл разбирается прямо в браузере и никуда не отправляется, парсер подгружается только в момент выбора файла.
